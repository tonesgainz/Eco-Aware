...
fetch("../../data/globe/ne_110m_admin_0_countries.geojson")
  .then((res) => res.json())
  .then((countries) => {
    // air exposure data set
    fetch("../../data/air/air_exposure.json")
      .then((res) => res.json())
      .then((data) => {
        ...
        function getFillColour(feat) {
          ...
          data.forEach((index) => {
            if (index.CODE == feat.properties.ISO_A3) value = index.VALUE;
          });
        ...
        myGlobe
          ...
          .onPolygonHover((hoverD) =>
            myGlobe
              ...
              .polygonCapColor((d) =>
                d === hoverD ? "steelblue" : getFillColour(d)
              )
          )
...
fetch(
  "https://api.openaq.org/v2/locations?limit=500&page=1&offset=0&sort=desc&radius=500&order_by=firstUpdated&dumpRaw=false",
  options
)
  ...
  .then((response) => {
    ...
    myGlobe
      .hexBinPointsData(response.results.filter((d) => d.coordinates !== null)) // don't put any null coordinates in the globe
